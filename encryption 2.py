...
def caesar_encrypt(text):
    key = random.randint(-5, 5)#some funky stuff may happen if deviate too far 
    write_key(str(key), 0)
...
def key_decrypt(text):
    ...
    return "".join([chr(ord(text[i])-keything[i]) for i in range(len(text))]) #subtract the corresponding num in the keylist from each character in the string
...
def fernet_decrypt(text):
    print("fernet" + text)
    key_object = Fernet(bytes(read_key(2), "utf-8")) #read the key and turn into byte, then turn into Fenet key object
...
#print(encrypt("Dunne!@#$%^&*()_+1111!@#$%^&*()_+1000"))